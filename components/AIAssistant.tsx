...
export default function AIAssistant() {
  ...
  const send = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/ai/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: userMsg.content, history: messages }) })
      const data = await res.json()
      setMessages(prev => [...prev, { role: 'assistant', content: data.reply }])
    } catch {
      setMessages(prev => [...prev, { role: 'assistant', content: 'Connection error. Please ensure Ollama is running.' }])
    }
    setLoading(false)
  }
    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-24 right-6 w-80 h-[480px] bg-white rounded-2xl shadow-2xl border border-[#f0e8de] flex flex-col z-50 overflow-hidden">
          <div className="px-4 py-3 border-b border-[#f0e8de] flex items-center gap-2" style={{ background: 'linear-gradient(135deg, #fdf5eb, #fff8f0)' }}>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm leading-relaxed ${
                  msg.role === 'user' ? 'text-white rounded-br-sm' : 'bg-[#fdf5eb] text-[#2c1810] rounded-bl-sm border border-[#f0e8de]'
          ...
          <div className="p-3 border-t border-[#f0e8de] flex gap-2">
            <input className="flex-1 border border-[#e5ddd4] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#c9a96e]"
              placeholder="Ask about your wedding..." value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} />
            <button onClick={send} disabled={loading} className="w-9 h-9 rounded-xl flex items-center justify-center disabled:opacity-50" style={{ background: 'linear-gradient(135deg, #c9a96e, #a07840)' }}>
