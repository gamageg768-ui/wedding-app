...
export default function Landing() {
  ...
  const { role, loginAsGuest, enterCouple } = useAuth()

  useEffect(() => {
    if (role === 'couple') navigate('/dashboard', { replace: true })
    ...
    <div className="min-h-screen bg-[#f9f5f0] flex flex-col items-center justify-center px-4">
      ...
      <div className="flex flex-col sm:flex-row gap-6 w-full max-w-md">
        ...
        <div className="flex-1 bg-white border border-[#e8d5b0] rounded-2xl shadow-sm overflow-hidden">
          <button
            onClick={handleGuest}
            className="w-full p-8 text-center hover:bg-[#fdfaf7] transition-colors group"
          >
          ...
          <div className="border-t border-[#f0e8de]">
            <button
              ...
              className="w-full py-3 px-4 text-xs text-[#b09070] hover:text-[#a07840] hover:bg-[#fdfaf7] transition-all flex items-center justify-center gap-1.5"
            >
              <ArrowRight size={12} />
