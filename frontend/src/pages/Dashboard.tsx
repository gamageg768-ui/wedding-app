...
import { Users, CheckSquare, DollarSign, Store, Heart, Clock } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const plannerPct = plannerStats && plannerStats.total > 0
    ? Math.round((plannerStats.completed / plannerStats.total) * 100) : 0
  const budgetPct = budgetSummary && budgetSummary.total_actual > 0
    ? Math.round((budgetSummary.total_paid / budgetSummary.total_actual) * 100) : 0
  ...
  return (
    <div className="p-8">
      ...
      {/* Countdown + Venue Hero */}
      {invitation?.wedding_date && (
        <div className="card mb-6 flex items-center justify-between"
             style={{ background: 'linear-gradient(135deg, #fdf5eb, #fff8f0)', borderColor: '#e8d5b0' }}>
          <div>
      ...
      {!invitation?.wedding_date && (
        <div className="card mb-6 text-center py-8 border-dashed border-2 border-[#e8d5b0]">
          ...
          <Link to="/invitation" className="btn-gold text-sm">Create Invitation</Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        {[
          {
            icon: Users, label: 'Guests Confirmed', value: guestStats?.confirmed ?? '—',
            sub: `${guestStats?.total ?? 0} total · ${guestStats?.pending ?? 0} pending`,
            color: 'text-blue-600', bg: 'bg-blue-50', to: '/guests'
          },
          {
            icon: CheckSquare, label: 'Tasks Done', value: `${plannerPct}%`,
            sub: `${plannerStats?.completed ?? 0} of ${plannerStats?.total ?? 0} tasks`,
            color: 'text-green-600', bg: 'bg-green-50', to: '/planner'
          },
          {
            icon: DollarSign, label: 'Budget Paid', value: `${budgetPct}%`,
            sub: `$${(budgetSummary?.total_paid ?? 0).toLocaleString()} of $${(budgetSummary?.total_actual ?? 0).toLocaleString()}`,
            color: 'text-amber-600', bg: 'bg-amber-50', to: '/budget'
          },
          {
            icon: Store, label: 'Attending', value: guestStats?.total_attending ?? '—',
            sub: `Including plus-ones`,
            color: 'text-rose-600', bg: 'bg-rose-50', to: '/guests'
          }
        ].map(({ icon: Icon, label, value, sub, color, bg, to }) => (
          <Link key={label} to={to} className="card hover:shadow-md transition-shadow cursor-pointer">
        ...
        <div className="card">
          ...
          <div className="w-full bg-[#f0e8de] rounded-full h-3">
            <div className="h-3 rounded-full transition-all duration-700"
                 style={{ width: `${plannerPct}%`, background: 'linear-gradient(90deg, #c9a96e, #a07840)' }} />
          </div>
        ...
        <div className="card">
          ...
          <div className="flex h-3 rounded-full overflow-hidden bg-[#f0e8de]">
            <div className="bg-green-400 transition-all duration-700"
                 style={{ width: `${guestStats ? (guestStats.confirmed / Math.max(guestStats.total, 1)) * 100 : 0}%` }} />
            <div className="bg-red-300 transition-all duration-700"
                 style={{ width: `${guestStats ? (guestStats.declined / Math.max(guestStats.total, 1)) * 100 : 0}%` }} />
          </div>
      ...
      <div className="mt-6">
        ...
        <div className="flex flex-wrap gap-3">
          {[
            { to: '/invitation', label: '✉️ Edit Invitation' },
            { to: '/guests', label: '👥 Add Guest' },
            { to: '/planner', label: '✅ View Tasks' },
            { to: '/budget', label: '💰 Track Budget' },
            { to: '/vendors', label: '🏪 Add Vendor' },
            { to: '/seating', label: '🪑 Seating Chart' },
          ].map(({ to, label }) => (
            <Link key={to} to={to} className="btn-outline text-sm">{label}</Link>
          ))}
        </div>
      </div>
    </div>
